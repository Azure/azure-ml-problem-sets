"""
The Azure ML pipeline for running a basic experiment for which the graph is dynamically generated based on a parameter value.
...
class DynamicGraphDemo(AMLPipelineHelper):
    ...
    def build(self, config):
        ...
        def demo_pipeline_function():
            ...
            if (which_component == "helloworld"):
                component_step = hello_world_component()
                self.apply_recommended_runsettings(
                "HelloWorldComponent", component_step, gpu=False
            )
            elif (which_component == "democomponent"):
                component_step = component_with_parameter(
                    value=config.democomponent.value
                )
                self.apply_recommended_runsettings(
                    "ComponentWithParameter", component_step, gpu=False
                )  
